...
public:
    ...
    float Max_velocity; //Maximum velocity in KM/hour, ideal velocity i.e. what the plane can be expected to spend most of a flight at when not in takeoff/landing is 90% of this. //TODO: Should averageVelocity(Float) be a value in itself? Depends on how often we need to calculate it.
    ...
    int Onboard (vector of passenger objects); //Count of passengers //TODO: Make this *actually* a passenger vector, not just an int
    int Count_of_passengers; // Number of passengers onboard

    //Availability bools
    bool In_transit (Bool); //Is the plane in flight?
    bool Is_operable (Bool); //Can the plane currently be used for transport?
