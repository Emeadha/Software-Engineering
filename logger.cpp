...
using namespace std;
namespace logger {
void Logger::logPlaneUpdate(const std::string& pid, int p_status) {
...
void Logger::logFlightUpdate(const std::string& fid, int f_status) {
    // Determine the update message based on the status
    if (f_status == 1) {
...
void Logger::logAirportUpdate(const std::string& aid, int a_status) {
    // Determine the update message based on the status
    if (a_status == 1) {
...
void Logger::exportLogsToFile(const std::string& filename) {
    ofstream file(filename);
    if (file.is_open()) {
        file << "Status log update:\n";

        // Write plane log with label
        file << "Plane status: " << plane_log << endl;

        // Write airport log with label
        file << "Airport status: " << airport_log << endl;

        // Write flight log with label
