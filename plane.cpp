...
Plane::Plane(int Plane_ID, string Plane_name, string Plane_model, float Max_fuel, float Burn_rate, 
    ...
    {
        Log_object->errorLog(1, "Error! Invalid initialization parameters for Plane object [plane.cpp][Line 19]");
    }
...
void Plane::setLogObject(Logger *log_pointer){
    ...
    {
        Log_object->errorLog(1, "Error! Null logger pointer provided [plane.cpp][Line 101]");
    }
...
double Plane::findDuration(Clock& new_time){
    // Check for invalid new_time parameter
    if (new_time.hours < 0 || new_time.minutes < 0 || new_time.seconds < 0) 
    {
        Log_object->errorLog(1, "Error! Invalid time parameter provided for duration calculation [plane.cpp][Line 112]");
    }
...
void Plane::planeStatus(){
    ...
    {
        Log_object->errorLog(1, "Error! Plane state neither flying nor grounded [plane.cpp][Line 127]");
    }
...
void Plane::checkFuelLevel(){
    ...
    {
        Log_object->errorLog(1, "Error! Invalid plane model for fuel level check [plane.cpp][Line 304]");
    }
...
void Plane::doMaintenance()
{
    ...
    if (untilMaintDone < 0) 
    {
        Log_object->errorLog(1, "Error! Unexpected maintenance duration calculation [plane.cpp][Line 475]");
    }
...
void Plane::sendToMaintenance(){
     // Add error logging for unexpected issues with initiating maintenance
     if (untilMaintDone < 0) 
     {
        Log_object->errorLog(1, "Error! Unexpected issue initiating maintenance [plane.cpp][Line 496]");
     }
